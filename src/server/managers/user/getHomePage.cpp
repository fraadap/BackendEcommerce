...
void UserManager::getHomePage(redisContext *redis, int clientSocket){
    std::string articoli;
    try{
        std::vector<Articolo> home_page = this->db->get_home_page();
        if (home_page.size() > 0){
            articoli = home_page[0].getNome();
        }
        for (int x = 1; x < home_page.size(); ++x) {
            articoli += home_page[x].getNome();
            articoli += "\n";
        }
     ...
     // Crea una stringa di risposta che include l'ID e l'username
    std::string responseBody = articoli;
