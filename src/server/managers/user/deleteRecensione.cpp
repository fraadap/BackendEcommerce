...
void UserManager::deleteRecensione(redisContext *redis, int clientSocket, int reviewId){
    Recensione r(reviewId);
    try{
        this->db->delete_recensione(r);
     ...
     // Crea una stringa di risposta che include l'ID e l'username
    std::string responseBody = "recensione numero " + intToString(reviewId) + " cancellata con successo";
