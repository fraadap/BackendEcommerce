...
void UserManager::addReview(redisContext *redis, int clientSocket, int idUtente, int itemId, int valutazione, const char *descrizione){

    Utente u(idUtente);
    Articolo a(itemId);
    Recensione r(0, u, a, "", valutazione, descrizione);

    try {
        this->db->create_recensione(r);
