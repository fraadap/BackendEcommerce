...
void UserManager::deleteFavorite(redisContext *redis, int clientSocket, int favoriteId){
    Lista_desideri r(favoriteId);
    try{
        this->db->delete_lista_preferiti(r);
     ...
     // Crea una stringa di risposta che include l'ID e l'username
    std::string responseBody = "favorite numero " + intToString(favoriteId) + " cancellata con successo";
