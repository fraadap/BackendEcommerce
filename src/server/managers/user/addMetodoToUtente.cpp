...
void UserManager::addMetodoToUtente(redisContext *redis, int clientSocket, int idUtente, int paymentid){
    Metodo_pagamento mp(paymentid);
    Utente ut(idUtente);
    Utente_metodo um(0, ut, mp, "");
    try{
        um = this->db->add_metodo_pagamento_to_utente(um);
